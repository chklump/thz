...
# Normal devices
  ...
  $hash->{AttrList}= "IODev do_not_notify:1,0  ignore:0,1 dummy:1,0 showtime:1,0 "
		    ...
		    ."interval_sFan:0,60,120,180,300,600,3600,7200,43200,86400 "
...
sub THZ_Define($$) {
  ...
  THZ_Refresh_all_gets($hash) if($init_done);
...
########################################################################################
#
# THZ_Notify - used at startup to delay THZ_Refresh_all_gets after init_done
# 
#
########################################################################################
...
sub THZ_Notify($$) {
  ...
  if ($dev->{NAME} eq "global" && grep (m/^INITIALIZED|REREADCFG$/, @{$dev->{CHANGED}})) {
    #Log3 $hash->{NAME}, 3, "thzversion = $thzversion ";
...
sub THZ_Get_Comunication($$) {
    ...
    # ---------step1 --> send request   SOH start of heading -- Null 	-- ?? -- DLE data link escape -- EOT End of Text  
    THZ_Write($hash,  $cmdHex);
    ($err, $msg) = THZ_ReadAnswer($hash);
    ...
    ($err, $msg) = THZ_ReadAnswer($hash);	
    THZ_Write($hash,  "10");  
    if  (defined($err))     {$err .= " THZ_Get_Com: error found at step2"; select(undef, undef, undef, 0.1);return($err, $msg) ;} 
    # Expectedanswer2 // read from the heatpump
...
########################################################################################
# only for debug; start with "get Mythz debug_read_raw_register_slow"
#
########################################################################################
sub THZ_debugread($){
    my ($hash) = @_;
	...
	#$cmd="0B013D00D2"; #set temp standby to D2hex = 210 dec to be decided by 10 --> 21°C
	#$cmd="D10000"; #delete last 10 errors 
	#my $cmdHex2 = THZ_encodecommand($cmd,"set",);
        #($err, $msg) = THZ_Get_Comunication($hash,  $cmdHex2);
        #STX start of text
        THZ_Write($hash,  "02");
        ($err, $msg) = THZ_ReadAnswer($hash);
